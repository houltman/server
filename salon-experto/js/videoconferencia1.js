...
function handleWindowResize() {
   ...
   // alert(connectCount);
  // var esta=window.innerWidth-20%;
  var alto = parseFloat(window.innerHeight);
var total =Math.floor(alto- ((50 * alto)/100) );
alert(total);
//alert (esta);
    ...
    if (connectCount>0)
{
    ...
    //fullpage.style.height = window.innerHeight + "px";
      fullpage.style.height = total + "px";

   // alert(fullpage.style.height);
...
function setReshaper(elementId, reshapeFn) {
    ...
    if( !element) {
        alert("Intentar aplicar para cambiar el elemento no existente " + elementId);
    }
    if( !reshapeFn) {
        alert("Intento de aplicar nombre incorrecto al elemento " + elementId);
    }
...
function appInit() {
    ...
    easyrtc.setDisconnectListener( function() {
        easyrtc.showError("LOST-CONNECTION", "Servidor Desconectado!");
    });
